...
\long\def\@xs@removefirstsyntaxunit#1#2{%
	...
	\expandafter\expandafter\expandafter\ifx\expandafter\expandafter\expandafter\@empty\expandafter\@xs@beforespace\detokenize\expandafter{\@xs@argument} \@xs@nil\@empty% #1 commence par un espace ?
		\expandafter\@xs@ifempty\expandafter{\@xs@argument}%
			{\let#2\@empty}
			{\afterassignment\@xs@testsecondtoken% aprÃ¨s avoir mangÃ© le 1er token, on va tester si la suite commence par Â«{Â»
			\expandafter\let\expandafter\@xs@secontoken\expandafter=\expandafter\@sptoken\@xs@argument\@xs@@nil\@xs@nil% on mange le 1er token et on rajoute \@xs@@nil Ã  la fin pour Ã©viter de perdre les accolades du groupe
...
% #1 : num\'ero du 1er argument; #2 : nombre de lignes; #3 argument optionnel; #4 : dernier num\'ero de l'argument devant Ãªtre d\'etokenis\'e si \'etoile
...
% #1 : dernier num\'ero de l'argument devant Ãªtre d\'etokenis\'e
...
% \@xs@atbegingroup      (un groupe vient d'Ãªtre ouvert)
...
\@xs@newmacro\IfInteger{}{1}{0}{%
	...
	\ifx\@empty\@xs@afterdecimal
		\ifnum\decimalpart=\z@
			\let\@xs@next\@firstoftwo% partie dÃ©cimale constituÃ©e de 0 --> seul cas oÃ¹ on renvoie vrai
		\else
...
% Renvoie ce qui est \`a gauche de l'occurence nÂ°#1 de la sous chaine #3 dans la chaine #2
\@xs@newmacro*3\StrBefore{1}{2}{1}{%
	\@xs@cutafteroccur{#2}{#3}{#1}%
	\expandafter\@xs@ReturnResult\expandafter{\@xs@reserved@C}{#4}%
}

% Renvoie ce qui est \`a droite de l'occurence nÂ°#1 de la sous chaine #3 dans la chaine #2
\@xs@newmacro*3\StrBehind{1}{2}{1}{%
	\@xs@cutafteroccur{#2}{#3}{#1}%
	\expandafter\@xs@ReturnResult\expandafter{\@xs@reserved@E}{#4}%
}

% Renvoie ce qui est strictement compris entre les occurrences nÂ°#1 et nÂ°#2
...
% Cette macro coupe la chaine #1 apr\`es l'unit\'e syntaxique nÂ°#2
\def\StrSplit{%
	\@ifstar
...
% \macroA reÃ§oit ce qui se trouve avant la n Ã¨me occurrence <motif> dans <texte>
% \macroB reÃ§oit ce qui est aprÃ¨s cette n Ã¨me occurrence
...
%      Teste si une chaÃ®ne est \'egale \`a plusieurs cha\^ines donn\'ee et
...
%      Compare 2 chaÃ®nes et renvoie la position du 1er caract\`ere diff\'erent.
...
%     num\'erique est <1 : la 1ere coupure doit Ãªtre vide et le seconde doit
%     Ãªtre \'egale \`a l'argument.
%------------------------------------------------------------------------------
% v1.4b  16/11/2008
...
%    Ãªtre d\'etok\'enis\'e dans le cas d'une macro \'etoil\'ee (sachant que
%     l'argument optionnel ne l'est jamais)
%------------------------------------------------------------------------------
% v1.5a  09/2/2009
%  - Nouvelle macro \StrExpand, sensible au mode d'exploration. La macro est
%    indÃ©pendante du mode d'exploration et dispose de ses propres commandes
%    d'exploration : \expandingroups et \noexpandingroups
%  - R\'e-\'ecriture de \scancs, qui devient triviale puisque d\'ependant de
%    \StrExpand. Il n'y a plus de version \'Ã©toil\'ee et d\'esormais, \scancs
...
%    non pas l'inverse pour que \@xs@toks reflÃ¨te ce qui se trouve dans #1
%  - Correction d'un bug dans \@xs@returnfirstsyntaxunit au cas oÃ¹ #1 commence
%    par un espace : mettre \@xs@toks Ã  0
%  - Correction d'un bug dans \@xs@returnfirstsyntaxunit :
%    \@xs@returnfirstsyntaxunit{{}}\truc
%    faisait que \truc se dÃ©veloppait en un espace.
%  - Correction d'un bug dans \@xs@removefirstsyntaxunit oÃ¹ le cas
%    \@xs@removefirstsyntaxunit{ {b}}\truc
%    donnait \truc se dÃ©veloppant en Â«bÂ» au lieu de Â«{b}Â».
%------------------------------------------------------------------------------
% v1.5d  28/3/2010
...
%  - \@xs@cutafteroccur@i ne produit plus d'espace indÃ©sirable si #3 Ã©tait
%    Ã©valuÃ© par \numexpr
%------------------------------------------------------------------------------
% v1.7  13/1/2013
...
%------------------------------------------------------------------------------
% v1.7b 29/7/2013
%  - La date contenue dans \xstringdate est dÃ©sormais conforme
%------------------------------------------------------------------------------
% v1.7c 13/10/2013 
%  - les macros impliquÃ©es dans \IfStrCase et \IfStrEqCase sont dÃ©sormais \long
